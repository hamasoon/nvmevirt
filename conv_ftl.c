...
static inline bool check_flush_buffer(struct buffer *buf)
{
	...
	return full_ppgs_cnt >= buf->ppg_per_buf / 2;
#elif (FLUSH_TIMING_POLICY == HALF_STATIC || FLUSH_TIMING_POLICY ==  HALF_WATERMARK)
	return full_ppgs_cnt >= buf->buffer_high_watermark;
#endif
}
...
static inline bool check_flush_buffer_allocate_fail(struct buffer *buf)
{
	...
	return false;
#elif (FLUSH_TIMING_POLICY == FULL)
	return full_ppgs_cnt >= buf->ppg_per_buf;
#elif (FLUSH_TIMING_POLICY == FULL_WAIT_HALF || FLUSH_TIMING_POLICY == FULL_WAIT_QUATER)
	return full_ppgs_cnt >= buf->flush_threshold;
#elif (FLUSH_TIMING_POLICY == HALF_NAIVE || FLUSH_TIMING_POLICY == HALF_WATERMARK || FLUSH_TIMING_POLICY == HALF_STATIC)
	return full_ppgs_cnt >= buf->buffer_high_watermark;
#endif
}
...
#if (FLUSH_TIMING_POLICY == IMMEDIATE)
	return;
#elif (FLUSH_TIMING_POLICY == FULL || FULL_WAIT_QUATER || FULL_WAIT_HALF)
