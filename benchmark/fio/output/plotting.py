            ...
            for s in sync:
                for sz in size:
                    ...
                    with open(filepath, 'r') as f:
                        ...
                        if target_data == 'clat_ns':
                            data.at[sz, f'{r}-{rw_type}-{s}'] = float(raw_data['jobs'][0][rw_type]['clat_ns']['mean'])
                        elif target_data == 'bw':
...
def plot_data(data1, data2, data3, random='rand', rw_type='read', sync='sync', x_label='Block Size(Byte)', y_label='BW (MB/s)'):
    size = ['4K', '8K', '16K', '32K', '64K', '128K', '256K', '512K', '1M']
    data_type = f'{random}-{rw_type}-{sync}'
