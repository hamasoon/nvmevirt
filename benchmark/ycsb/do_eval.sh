...
function do_init() {
	...
	sudo chown layfort:layfort ${DB_PATH}
...
./bin/ycsb.sh run rocksdb -s -P workloads/workloada -p rocksdb.dir="/mnt/nvme/" -p rocksdb.optionsfile=rocksdb-options.ini -p maxexecutiontime=600 -p status.interval=1