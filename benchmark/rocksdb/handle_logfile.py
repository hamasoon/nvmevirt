...
class log_data:
    ...
    def read_log_data(self):
        filename = os.path.join(os.path.dirname(__file__), f"{self.policy}/{self.mapping}.mesg.log")
...
def plot_write_per_time(raw_data: dict):
    ...
    for policy in POLICY:
        ...
        for mapping in MAPPING:
            data[policy][mapping] = raw_data[policy][mapping].w_total_size / raw_data[policy][mapping].runtime / 1024 / 1024
...
def plot_read_per_time(raw_data: dict):
    ...
    for policy in POLICY:
        data[policy] = {}
        for mapping in MAPPING:
            data[policy][mapping] = raw_data[policy][mapping].r_total_size / raw_data[policy][mapping].runtime
    ...
    plt.ylabel(f'Byte/Sec', fontsize=12)
    plt.xticks(np.arange(len(POLICY)), POLICY, rotation=45, fontsize=8)
    plt.legend(title="Mapping", loc="lower right", fontsize=10)
    plt.tight_layout()
...
def plot_write_hit_ratio(raw_data: dict):
    ...
    for policy in POLICY:
        data[policy] = {}
        for mapping in MAPPING:
            data[policy][mapping] = raw_data[policy][mapping].write_hit_size / (raw_data[policy][mapping].w_total_size / MAPPING_SIZE[mapping])
...
if __name__ == '__main__':
    ...
    for policy in POLICY:
        data[policy] = {}
        for mapping in MAPPING:
            data[policy][mapping] = log_data(policy, mapping)
