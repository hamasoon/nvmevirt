        ...
        for bs in "${BS[@]}"; do
            for sync in "${SYNC[@]}"; do
                for rand in "${RAND[@]}"; do
                    ...
                    if [ "$rw" == "read" ]; then
                        sudo dd if=/dev/zero of=/dev/nvme3n1 bs=32k count=300000
                    fi
                    ...
                    if [ "$sync" == "sync" ]; then
                        sudo DEV="/dev/${DEV}" JOB=1 DEPTH=1 \
                            fio "$workload" --bs=${bs} --output-format=json --ioengine=psync \
                                --output="output/${map}/${rand}${rw}-${sync}-${bs}.json"
                    else
                        sudo DEV="/dev/${DEV}" JOB="$JOBS" DEPTH="$DEPTH" \
                            fio "$workload" --bs=${bs} --output-format=json --ioengine=io_uring \
                                --output="output/${map}/${rand}${rw}-${sync}-${bs}.json"
                    fi
